import React, { useState, useMemo, useEffect } from 'react';
import { Educator, EducatorStatus, WhatsAppGroup } from '../../types';
import { generateDocumentText } from '../../services/geminiService';

interface BulkEducatorMeetingModalProps {
  educators: Educator[];
  onClose: () => void;
  onSchedule: (meetingData: any) => void;
}

const BulkEducatorMeetingModal: React.FC<BulkEducatorMeetingModalProps> = ({ educators, onClose, onSchedule }) => {
    const [schoolParticipants, setSchoolParticipants] = useState('Coordenação');
    const [subject, setSubject] = useState('');
    const [dateTime, setDateTime] = useState('');
    const [message, setMessage] = useState('');
    const [isGenerating, setIsGenerating] = useState(false);
    const [channels, setChannels] = useState({ email: true, whatsapp: true, app: true });
    const [savedGroups, setSavedGroups] = useState<WhatsAppGroup[]>([]);
    const [selectedGroupId, setSelectedGroupId] = useState<string>('');

    const activeEducators = useMemo(() => educators.filter(e => e.status === EducatorStatus.ACTIVE), [educators]);
    const isFormValid = subject && dateTime;
    const isGroupMode = !!selectedGroupId;

    useEffect(() => {
        const storedGroups = localStorage.getItem('whatsapp_groups');
        if (storedGroups) {
            setSavedGroups(JSON.parse(storedGroups));
        }
    }, []);

    const handleGenerateMessage = async () => {
        if (!isFormValid) {
            alert("Preencha o assunto e a data/hora antes de gerar a mensagem.");
            return;
        }
        setIsGenerating(true);
        const prompt = `Escreva uma mensagem de convite formal e amigável para uma reunião escolar em grupo para a equipe de educadoras.
        - Convidado: Equipe de Educadoras
        - Assunto da reunião: ${subject}
        - Data e Hora: ${new Date(dateTime).toLocaleString('pt-BR')}
        - Participantes da escola: ${schoolParticipants}
        A mensagem deve incluir um placeholder para um link de confirmação, como "[LINK_PARA_CONFIRMAR]".`;
        try {
            const result = await generateDocumentText(prompt);
            setMessage(result);
        } catch (error) {
            alert(`Erro ao gerar mensagem: ${error instanceof Error ? error.message : "Tente novamente."}`);
        } finally {
            setIsGenerating(false);
        }
    };
    
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!isFormValid) return;

        const finalMessage = message || `Lembrete de reunião: ${subject} em ${new Date(dateTime).toLocaleString('pt-BR')}.`;

        if (isGroupMode) {
            const selectedGroup = savedGroups.find(g => g.id.toString() === selectedGroupId);
            if (selectedGroup) {
                try {
                    await navigator.clipboard.writeText(finalMessage);
                    window.open(selectedGroup.link, '_blank');
                    onSchedule({
                        title: subject,
                        schoolParticipants,
                        date: new Date(dateTime).toISOString(),
                        isGroupMeeting: true,
                        groupName: selectedGroup.name,
                    });
                    return; 
                } catch (err) {
                    alert('Falha ao copiar a mensagem. O link do grupo será aberto.');
                    window.open(selectedGroup.link, '_blank');
                    onSchedule({
                        title: subject,
                        schoolParticipants,
                        date: new Date(dateTime).toISOString(),
                        isGroupMeeting: true,
                        groupName: selectedGroup.name,
                    });
                    return;
                }
            }
        }

        const meetingData = {
            title: subject,
            schoolParticipants,
            date: new Date(dateTime).toISOString(),
            channels: channels,
            message: finalMessage,
            isBulk: true
        };
        onSchedule(meetingData);
    };

    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 animate-fade-in" onClick={onClose}>
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/50 w-full max-w-2xl flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Convidar Todas as Educadoras</h2>
                    <button type="button" onClick={onClose} className="p-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </header>
                <main className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg text-center text-sm text-blue-800 dark:text-blue-200">
                        O convite será enviado para todas as <strong>{activeEducators.length}</strong> educadoras ativas.
                    </div>
                    
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="schoolParticipants" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Participantes da Escola</label>
                            <input type="text" id="schoolParticipants" value={schoolParticipants} onChange={e => setSchoolParticipants(e.target.value)} className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg"/>
                        </div>
                        <div>
                            <label htmlFor="dateTime" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data e Hora *</label>
                            <input type="datetime-local" id="dateTime" value={dateTime} onChange={e => setDateTime(e.target.value)} required className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg"/>
                        </div>
                    </div>
                    <div>
                        <label htmlFor="subject" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Assunto Principal *</label>
                        <input type="text" id="subject" value={subject} onChange={e => setSubject(e.target.value)} required className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg"/>
                    </div>

                    <div>
                        <label htmlFor="groupSelect" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Enviar para Grupo do WhatsApp</label>
                        <select id="groupSelect" value={selectedGroupId} onChange={e => setSelectedGroupId(e.target.value)} className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg">
                            <option value="">Não (enviar individualmente)</option>
                            {savedGroups.map(group => (
                                <option key={group.id} value={group.id}>{group.name}</option>
                            ))}
                        </select>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Selecionar um grupo irá direcionar o envio para o WhatsApp.</p>
                    </div>

                     <div>
                        <div className="flex justify-between items-center mb-1">
                            <label htmlFor="message" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Mensagem do Convite</label>
                            <button type="button" onClick={handleGenerateMessage} disabled={isGenerating} className="px-2 py-1 text-xs font-semibold bg-gray-200 dark:bg-gray-600/50 text-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 flex items-center">
                                {isGenerating ? 'Gerando...' : 'Gerar com IA ✨'}
                            </button>
                        </div>
                        <textarea id="message" value={message} onChange={e => setMessage(e.target.value)} rows={5} placeholder="Opcional. Uma mensagem padrão será usada se este campo ficar em branco." className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg"/>
                    </div>

                    <div>
                        <label className={`block text-sm font-medium mb-2 ${isGroupMode ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'}`}>Canais de Envio (Individual)</label>
                        <div className="flex space-x-4">
                            <label className={`flex items-center ${isGroupMode ? 'cursor-not-allowed' : ''}`}>
                                <input type="checkbox" checked={channels.email} onChange={e => setChannels(c => ({...c, email: e.target.checked}))} disabled={isGroupMode} className="h-4 w-4 rounded text-teal-500 disabled:bg-gray-400" />
                                <span className={`ml-2 ${isGroupMode ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'}`}>E-mail</span>
                            </label>
                            <label className={`flex items-center ${isGroupMode ? 'cursor-not-allowed' : ''}`}>
                                <input type="checkbox" checked={channels.whatsapp} onChange={e => setChannels(c => ({...c, whatsapp: e.target.checked}))} disabled={isGroupMode} className="h-4 w-4 rounded text-teal-500 disabled:bg-gray-400" />
                                <span className={`ml-2 ${isGroupMode ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'}`}>WhatsApp</span>
                            </label>
                            <label className={`flex items-center ${isGroupMode ? 'cursor-not-allowed' : ''}`}>
                                <input type="checkbox" checked={channels.app} onChange={e => setChannels(c => ({...c, app: e.target.checked}))} disabled={isGroupMode} className="h-4 w-4 rounded text-teal-500 disabled:bg-gray-400" />
                                <span className={`ml-2 ${isGroupMode ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'}`}>Aplicativo</span>
                            </label>
                        </div>
                    </div>
                </main>
                <footer className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                    <button type="submit" disabled={!isFormValid} className="px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-500 disabled:bg-gray-500">
                        {isGroupMode ? 'Agendar e Abrir Grupo' : 'Agendar e Enviar para Todas'}
                    </button>
                </footer>
                 <style>{`
                    @keyframes fade-in {
                        from { opacity: 0; transform: scale(0.98); }
                        to { opacity: 1; transform: scale(1); }
                    }
                    .animate-fade-in { animation: fade-in 0.2s ease-out forwards; }
                `}</style>
            </form>
        </div>
    );
};

export default BulkEducatorMeetingModal;