...
interface EnrollmentPaymentModalProps {
  applicant: Applicant;
  onClose: () => void;
  onConfirm: (updatedApplicant: Applicant) => void;
}

const EnrollmentPaymentModal: React.FC<EnrollmentPaymentModalProps> = ({ applicant, onClose, onConfirm }) => {
  ...
  const handleManualConfirmation = (method: 'PIX' | 'Dinheiro' | 'Cartão') => {
    onConfirm({
      ...
      paymentReceiptUrl: undefined, // Clear any previous receipt
    });
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/50 w-full max-w-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-baseline">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Confirmar Taxa de Matrícula</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 ml-2">de {applicant.name}</p>
            </div>
          <button onClick={onClose} className="p-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full ml-auto">
        ...
        <main className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
          ...
          <div className="space-y-4 flex flex-col">
            ...
            <div className="p-4 bg-gray-100 dark:bg-gray-900/50 rounded-lg flex-grow">
              <input type="file" id="receipt-upload-enroll" accept=".pdf,image/*" onChange={handleFileChange} className="hidden" />
              <label htmlFor="receipt-upload-enroll" className="cursor-pointer w-full h-full p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-500 dark:text-gray-400 hover:border-teal-500 hover:text-teal-400 transition-colors flex flex-col items-center justify-center text-center">
