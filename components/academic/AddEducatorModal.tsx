...
interface AddEducatorModalProps {
  ...
  onClose: () => void;
  onSave: (educator: SaveEducatorData) => void;
}
...
const AddEducatorModal: React.FC<AddEducatorModalProps> = ({ educatorToEdit, onClose, onSave }) => {
  const [name, setName] = useState('');
  const [role, setRole] = useState('Professora');
  const [subjectsStr, setSubjectsStr] = useState('');
  const [status, setStatus] = useState<EducatorStatus>(EducatorStatus.ACTIVE);
  const [hireDate, setHireDate] = useState(new Date().toISOString().split('T')[0]);
  const [avatar, setAvatar] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const isEditing = !!educatorToEdit;

  useEffect(() => {
    if (educatorToEdit) {
      setName(educatorToEdit.name);
      setRole(educatorToEdit.role);
      setSubjectsStr(educatorToEdit.subjects.join(', '));
      setStatus(educatorToEdit.status);
      setHireDate(educatorToEdit.hireDate);
      setAvatar(educatorToEdit.avatar);
    } else {
      // Reset form for new entry
      setName('');
      setRole('Professora');
      setSubjectsStr('');
      setStatus(EducatorStatus.ACTIVE);
  ...
  const hasChanges = useMemo(() => {
    if (!isEditing) {
        // For new educators, enable save if the name is filled
        return name.trim() !== '';
    }
    // For editing, check if any field has changed
    const subjectsChanged = subjectsStr !== (educatorToEdit?.subjects.join(', ') || '');
    return (
        name !== educatorToEdit?.name ||
        role !== educatorToEdit?.role ||
        subjectsChanged ||
        status !== educatorToEdit?.status ||
        hireDate !== educatorToEdit?.hireDate ||
        avatar !== educatorToEdit?.avatar
    );
}, [name, role, subjectsStr, status, hireDate, avatar, educatorToEdit, isEditing]);
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/50 w-full max-w-lg flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {isEditing ? 'Editar Educadora' : 'Adicionar Nova Educadora'}
          </h2>
        ...
        <main className="p-6 space-y-4">
           <div className="flex flex-col items-center space-y-2">
                {avatar ? (
                    <img src={avatar} alt="Foto da educadora" className="w-24 h-24 rounded-full object-cover" />
                ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200 dark:border-gray-700/50">
            ...
            <div>
                ...
                <input type="text" id="role" value={role} onChange={e => setRole(e.target.value)} required className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600" />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
                <label htmlFor="subjects" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Disciplinas (separadas por vírgula)</label>
                <input type="text" id="subjects" value={subjectsStr} onChange={e => setSubjectsStr(e.target.value)} placeholder="Ex: Matemática, Português" className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600" />
            </div>
            <div className="md:col-span-2">
                <label htmlFor="status" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Status</label>
                <select id="status" value={status} onChange={e => setStatus(e.target.value as EducatorStatus)} className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600">
                    <option value={EducatorStatus.ACTIVE}>Ativo</option>
                    <option value={EducatorStatus.INACTIVE}>Inativo</option>
                </select>
            </div>
          </div>
        </main>
        <footer className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
          ...
          >
            {isEditing ? 'Salvar Alterações' : 'Salvar Educadora'}
          </button>
        </footer>
         <style>{`
            @keyframes fade-in {
