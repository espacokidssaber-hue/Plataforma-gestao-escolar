...
const UploadContractModal: React.FC<UploadContractModalProps> = ({ onClose }) => {
    const { enrolledStudents, uploadSignedContract } = useEnrollment();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (studentId && file && fileUrl) {
            setIsUploading(true);
            // Simulate upload delay
            setTimeout(() => {
                uploadSignedContract(Number(studentId), file, fileUrl);
    ...
    const isFormValid = studentId && file;
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 animate-fade-in" onClick={onClose}>
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/50 w-full max-w-lg flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Enviar Contrato Assinado</h2>
                ...
                <main className="p-6 space-y-4">
                    <p className="text-sm text-gray-500 dark:text-gray-400">Selecione o aluno e anexe o arquivo do contrato digitalizado (PDF, JPG, PNG).</p>
                    <div>
                        <label htmlFor="studentId" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Aluno *</label>
                        <select id="studentId" value={studentId} onChange={e => setStudentId(e.target.value)} required className="w-full bg-gray-100 dark:bg-gray-700/50 p-2 rounded-lg text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <span className="text-sm">{file ? file.name : 'Clique para selecionar o arquivo'}</span>
                        </div>
                    </div>
                </main>
                <footer className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
                    ...
                    <button type="submit" disabled={!isFormValid || isUploading} className="px-4 py-2 w-28 bg-green-600 rounded-lg text-white font-semibold disabled:bg-gray-500 disabled:cursor-not-allowed hover:bg-green-500">
                        {isUploading ? (
